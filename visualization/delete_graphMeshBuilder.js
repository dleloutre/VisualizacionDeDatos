...
export class GraphMeshBuilder {
	...
	_createNodesMaterial(nodeSize = 15) {
		let mat = new THREE.ShaderMaterial({
			uniforms: {
	...
	createEdges(totalEdgedToDraw) {
		...
		console.log(
			"createEdgesGeometry() totalEdgedToDraw:" + totalEdgedToDraw
		);
		...
		// el valor va de 0.0 a 1.0
		const instanceRandomSeedArray = new Float32Array(totalEdgedToDraw * 1);
		...
		for (let i = 0; i < totalEdgedToDraw; i++) {
			// definimos la transformacion de cada instancia

			let res = this.graph.pickRandomEdge();
			...
			instanceGradientOffsetArray[i] = res.gradientOffset;
			instanceRandomSeedArray[i] = 4.0;
		}
		...
		//	console.log(instancedEdgeGeometry);
