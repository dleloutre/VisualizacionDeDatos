...
class MCGS(object):
    ...
    def _find_articulation_points(self, G):
        articulation_points = []
        for node in G.nodes():
            # Remove the node and calculate the number of weakly connected components
            G_removed = G.copy()
            G_removed.remove_node(node)
            num_components = len(list(nx.weakly_connected_components(G_removed)))
            # If removing the node increases the number of components, it's an articulation point
            if num_components > 1:
    ...
    def __identify_rim_and_tie_structures(self, G):
        ...
        cut_points = set(self._find_articulation_points(G))
