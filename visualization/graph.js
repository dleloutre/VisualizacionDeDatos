...
export class Graph {
  ...
  getNode(id) {
    ...
    let vTextureCoord =
      blockHeight / 2 + nodeVectorInfo.partyIndex / this.totalParties;
  ...
  getEdge(fromId, toId) {
    ...
    const gradientOffset = this.calculateGradientOffset(
      originPosition,
      targetPosition
    );
  ...
  calculateGradientOffset(originPosition, targetPosition) {
    return (
      (targetPosition.partyIndex +
        (1.0 + originPosition.partyIndex) / (1.0 + this.totalParties)) /
      this.totalParties
    );
  }
}
