...
const groupColors = {
  ...
  jadot: 0x59d5e0,
  asselineau: 0xc5ebaa,
  sandrousseau: 0xffbe98,
  kazib: 0x265073,
  taubira: 0xccd3ca,
  dupontaignan: 0xf72798,
  bertrand: 0xc68484,
  arthaud: 0x15f5ba,
  barnier: 0xb67352,
  montebourg: 0xfefbf6,
...
//     textmesh.position.x = labels[i].radius * 7 * Math.sin(labels[i].angle);
//     textmesh.position.y = labels[i].position.y * 7;
//     textmesh.position.z = labels[i].radius * 6 * Math.cos(labels[i].angle);
...
//   const labels = graph.getLabels();
//   console.log("labels:", labels);
//   const materialargs = { color: 0xffffff };

//   textMeshes = []; // Vacía el arreglo para los nuevos textos
...
//     scene.add(textmesh);
//     textMeshes.push(textmesh); // Añade el mesh al arreglo
//   }
// }
...
//   for (let i = 0; i < labels.length; i++) {
//     // Convertir el nombre del label a minúsculas y obtener el color correspondiente
//     let color = groupColors[labels[i].label.toLowerCase()] || 0xffffff; // Usa el color del grupo o blanco por defecto
//     let labelgeo = new TextGeometry(labels[i].label, {
//       font: font,
//       size: 5000,
//       depth: 10000 / 2,
...
let textMeshes = []; // Array para guardar las referencias a los textos
...
function drawGraph(graph, font) {
  ...
  for (let i = 0; i < labels.length; i++) {
    let color = groupColors[labels[i].label.toLowerCase()] || 0xffffff;
    let labelgeo = new TextGeometry(labels[i].label, {
      font: font,
      size: 10000,
      depth: 10000 / 2,
    ...
    //circular
    textmesh.position.x = labels[i].radius * 7 * Math.sin(labels[i].angle);
    textmesh.position.y = labels[i].position.y * 20;
    textmesh.position.z = labels[i].radius * 6.7 * Math.cos(labels[i].angle);
    textmesh.rotation.z = Math.PI / 2;
